...
document.getElementById('registroOngForm').addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    if (validarFormulario()) {
        ...
        console.log("Dados a serem enviados:", ong); // Log dos dados
        enviarDados(ong);
    }
});
...
async function enviarDados(ong) {
    try {
        const response = await fetch('http://localhost:8080/api/registro/ong', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                window.location.href = '/homeOng'; // Confirme este caminho
            }, 2000);
        } else {
